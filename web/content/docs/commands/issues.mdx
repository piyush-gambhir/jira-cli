...
Create reuses an issue made by the current user in the same project during the
last 10 minutes when its type, summary, and parent match exactly.
`--allow-duplicate` bypasses this protection.
